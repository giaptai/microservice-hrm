...
public interface LuongBanThanClient {
    ...
    List<LuongBanThanAuth> getAllByHoSoId(@PathVariable UUID id);

    @GetMapping(value = "/luong-ban-than")
    List<ResLuongBanThan> getAll();

    @GetMapping("/luong-ban-than/{id}")
    ResLuongBanThan getById(@PathVariable int id);

    @PostMapping("/luong-ban-than/{id}")
    ResLuongBanThan add(@PathVariable UUID id, @RequestBody ReqLuongBanThan cu);

    @PatchMapping("/luong-ban-than/{id}")
    ResLuongBanThan edit(@PathVariable int id, @RequestBody ReqLuongBanThan cu);
